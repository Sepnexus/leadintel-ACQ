#!/usr/bin/env bash
# Database-only first-boot initialization for ACQ Coach.
...
if [ -s "$PGDATA/PG_VERSION" ]; then
  ...
  for f in /docker-init/migrations/*.sql; do
    ...
    su -s /bin/bash postgres -c \
      "psql -h $SOCKETDIR -U postgres -d ${POSTGRES_DB:-acqcoach} -v ON_ERROR_STOP=0 -f $f" \
      >/dev/null 2>&1 || true
  done
...
# Allow the edge-runtime sidecar to read/write — same Docker bridge network
...
# pg_cron config (extension itself is enabled in post-migrations.sql)
shared_preload_libraries = 'pg_cron'
cron.database_name = '${POSTGRES_DB:-acqcoach}'
EOF
...
log "applying user migrations (Lovable's 28 ACQ migration files)"
for f in /docker-init/migrations/*.sql; do
  [ -f "$f" ] || continue
  log "  ↳ $(basename "$f")"
...
log "running post-migrations.sql (grants + pg_cron/pg_net extensions)"
PSQL_AS_PG "-f /docker-init/post-migrations.sql" "$POSTGRES_DB"

log "stopping init Postgres (start.sh will boot it for real, this time with pg_cron preloaded)"
